...
class Encoder(nn.Module):
    def __init__(self, vocab_size, d_model, N, heads, dropout):
        ...
        #这里的形状是(batchsize,长度,维度)==》卷积之后，(batchsize,长度减小,维度)
        self.conv1d = CnnLayer(config.d_model,config.cnn_size,config.cnn_step)
        self.mask_pool1d_1=nn.MaxPool1d(config.cnn_size,stride=config.cnn_step)#这里和卷积操作的参数保持一致,所以有两次
        self.mask_pool1d_2=nn.MaxPool1d(config.cnn_size,stride=config.cnn_step)
    def forward(self, src, mask):
        ...
        src_pool_mask=self.mask_pool1d_1(mask)
        src_pool_mask=self.mask_pool1d_2(src_pool_mask)

        return self.norm_1(x),self.norm_2(self.conv1d(x)),src_pool_mask
...
class Decoder(nn.Module):
    ...
    def forward(self, trg, e_outputs, src_mask, trg_mask, cnn_outputs, src_pool_mask):
        x = self.embed(trg)
        x = self.pe(x)
        for i in range(self.N):
            x = self.layers[i](x, e_outputs, src_mask, trg_mask,cnn_outputs,src_pool_mask)
        return self.norm(x)
...
class Transformer(nn.Module):
    ...
    def forward(self, src, trg, src_mask, trg_mask):
        e_outputs,cnn_outputs,src_pool_mask = self.encoder(src, src_mask)

        d_output = self.decoder(trg, e_outputs, src_mask, trg_mask, cnn_outputs, src_pool_mask)
    ...
    def prediction(self, src, src_mask,use_beam):
        ...
        if(not use_beam):
            for i in range(len):
                d_output_temp = self.decoder(trg, e_outputs, src_mask, trg_mask, cnn_outputs, src_pool_mask)
            ...
            for i in range(len):
                if(i==0):#第一次的话，只有一个开始结果,不用循环
                    ...
                    for j in range(config.beam_size):#计算出j情况下的可能性                   
                        ...
                        d_output_temp = self.decoder(beam_list[:,:,j], e_outputs, src_mask, trg_mask, cnn_outputs, src_pool_mask)
                        output_temp,indexs=torch.topk(torch.softmax(self.out(d_output_temp),-1),k=config.beam_size,dim=-1)
