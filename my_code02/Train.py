        ...
        for one in bar:
            ...
            for j in range(x.size(0)):
                if(tokenizer.decode(y[j,1:],skip_special_tokens=True)=="" or tokenizer.decode(y_pre[j],skip_special_tokens=True)==""):#验证集上有空数据，临时判断一下
                    ...
                    y_text=tokenizer.decode(y[j,1:],skip_special_tokens=True)
                    y_text_pre=tokenizer.decode(y_pre[j],skip_special_tokens=True)
                    y_list.append(y_text)
                    y_pre_list.append(y_text_pre)
                    
                    # print(y.split(" "))
                    # print([y_pre.split(" ")])
                    bleu_score+=sentence_bleu([y_text.replace(" ","")],y_text_pre.replace(" ",""))
                    meteor_score_num+=meteor_score([y_text.split(" ")],y_text_pre.split(" "))
                    nums+=1
                    print("sss")
        print("bleu分数",bleu_score/nums)
        print("meteor分数",meteor_score_num/nums)
...
def train(model,train_datas,dev_datas,optimizer,lossCompute,tokenizer,start_epoch,test_datas=None):
    ...
    for i in range(start_epoch,100):
        ...
        #保存模型和优化器，方便断点训练
        if(best_mean_rouge<=mean_rouge):#说明dev还在下降
